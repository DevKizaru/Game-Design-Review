...
- **Redundância sistêmica (paralelo)** — o recurso já tinha fonte e agora tem três. Quase toda proposta de sistema novo é uma porta nova para um recurso que já tem porta, e chega disfarçada de "um NPC que vende X", "uma daily que dá X". Some as fontes numa tabela **antes** de discutir valores: o dano não é uma delas estar desbalanceada, é o otimizador escolher uma e as outras virarem decoração que o jogador ainda precisa aprender. Se a moeda gasta numa é a que outra consome, há **circuito** — ou vira arbitragem, ou mata um sink. `modelagem.md` §10; `auditoria_calc.py fontes`.
- **Gargalo em série / cadeia obrigatória** — o oposto topológico da anterior, e a que melhor sobrevive ao review porque *parece* profundidade: cada elo tem nome, arte e tela próprios. Em paralelo o dano é uma fonte matar as outras; em série é o sistema de apoio virar gargalo do principal. A vazão é a do elo mais fraco (todo tuning fora do gargalo rende zero) e a confiabilidade **cai** a cada elo — três elos com 15% de ociosidade dão 39% de chance de a linha estar parada. Teste elo por elo: **“decisão ou só um passo?”** — sem escolha que mude resultado, sem risco próprio e sem saída com outro destino, é **pedágio**. Cadeia de N elos com N−1 pedágios é uma fonte única com N−1 telas de espera. `modelagem.md` §11; `auditoria_calc.py cadeia`.
...
- **Oferta que você não criou** — o preço de um recurso cai sem patch que explique. Não é o mercado "se ajustando": é faucet que você não desenhou, e as origens são bot, dupe, drop mal calibrado ou evento esquecido ligado. A conta que prevê bot é `preço de RMT ÷ recurso por hora`: se isso compensa um salário-hora em algum lugar do mundo, você vai ter bot, e isso é aritmética, não caráter do seu público. `references/mercado-e-monetizacao.md`.
- **Vantagem paga sem teto declarado** — a loja vende algo que vira poder e ninguém escreveu até onde. As três perguntas são `razão de poder`, `tempo de alcance` do não-doador e `fração paga` da economia. Nenhum valor é proibido; **não ter o valor escrito é o achado**, porque um servidor que não sabe dizer sua razão de poder já escolheu uma. E cuidado com o rótulo: se a compra pode ser expressa em horas por semana, é vantagem, não conveniência. `references/mercado-e-monetizacao.md`.
- **Patch auditado pelo diff** — auditar o buraco e ignorar a parede. O que quebra quase nunca está nas linhas alteradas: está no que foi calibrado *contra* o valor antigo e não foi tocado. `references/patch.md`.
